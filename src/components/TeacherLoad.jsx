    ...
    classes.forEach(cls=>{
      ...
      Object.entries(alloc).forEach(([sid, tid])=>{
        if (tid===t.id){
          ...
          if (!classSet.has(cls.id)){ learners += cls.learners; classSet.add(cls.id); }
        }
      });
    });
        ...
        <tbody>
          {loads.map(r => (
            <tr key={r.id} className="border-b last:border-0">
              ...
              <td className={`py-2 ${r.status==='Overload'?'text-[color:var(--warn)]': r.status==='Underload'?'text-[color:var(--accent)]':'text-emerald-600'}`}>{r.status}</td>
            </tr>
          ))}
        </tbody>
      </table>
