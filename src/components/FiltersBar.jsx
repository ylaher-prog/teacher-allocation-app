...
export default function FiltersBar(){
  ...
  return (
    <div style={{ maxWidth:980, margin:'8px auto', padding:'8px 16px', display:'flex', gap:12, alignItems:'center' }}>
      <label>Curriculum</label>
      <select value={filters.curriculum} onChange={e=>setFilters({ curriculum: e.target.value })} style={sel()}>
        {curricula.map(x => <option key={x} value={x}>{x}</option>)}
      </select>
      <label>Grade</label>
      <select value={filters.grade} onChange={e=>setFilters({ grade: e.target.value==='All'?'All':Number(e.target.value) })} style={sel()}>
        {grades.map(x => <option key={x} value={x}>{x}</option>)}
      </select>
      <label>Mode</label>
      <select value={filters.mode} onChange={e=>setFilters({ mode: e.target.value })} style={sel()}>
